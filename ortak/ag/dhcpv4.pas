{==============================================================================
  ...
  Dosya Adı: dhcpv4.pas
  Dosya İşlevi: DHCP v4 protokol işlevlerini yönetir

  Güncelleme Tarihi: 26/08/20256

  Bilgi: sadece kullanılan sabit, değişken ve işlevler türkçeye çevrilmiştir
...
const
  { TODO - dns sunucusu tarafından yapılandırılacak, şu aşamada dhcp sunucu tarafından
    gönderilecek değer olarak belirlenmiştir }
  // aşağıdaki 2 değer şu aşamada dhcp sunucusu tarafından kullanılmaktadır
...
const
  ...
  DHCP_SECIM_ZAMAN_OFFSET                 = TSayi1(2);      // kullanılmıyor
  ...
  DHCP_SIHIRLI_CEREZ                      = TSayi4($63825363);    // network byte sıralı kodlama
...
type
  ...
  public
    ...
    procedure DHCPTeklifMesajiGonder(AGonderenKimlik: TSayi4;
      ATeklifEdilenIPAdresi: TIPAdresIslev; AMACAdres: TMACAdres);
    { Request }
    ...
    procedure DHCPIstegeOnayMesajiGonder(AGonderenKimlik: TSayi4;
      AIstenenIPAdresi: TIPAdresIslev; AMACAdres: TMACAdresIslev);
    { Inform }
    ...
    procedure DHCPBilgilendirmeyeOnayMesajiGonder(AGonderenKimlik: TSayi4;
      AIPAdres: TIPAdresIslev; AMACAdres: TMACAdresIslev);
    { NAck }
...
{==============================================================================
  DHCP sunucusuna keşif mesajı gönderir
 ==============================================================================}
...
begin
  ...
  DHCPYapi := GetMem(4096);
	...
	DHCPYapi^.DonanimUz := 6;		        // mac uzunluğu
  ...
  // en sondaki işaretçi hariç (DigerSecenekler) yapı uzunluğu
  DHCPYapiUzunlugu := SizeOf(TDHCP4Yapi) - 4;

  // diğer seçenekler
  p1 := @DHCPYapi^.DigerSecenekler;

  // dhcp mesaj kodlaması
  // 1. byte = mesaj tip, 2. byte = mesajın uzunluğu, 3. byte = mesajın kendisi
  ...
  Inc(p1);
  p1^ := 1;
  Inc(p1);                                // donanım tipi = 1 (ethernet)
  ...
  B := GBaglantilar.BaglantiOlustur(itIP4, btAktif, ptUDP, IPAdresi, DHCP_ISTEMCI_PORT,
    DHCP_SUNUCU_PORT);
  if not(B = nil) then
  begin
...
{==============================================================================
  DHCP istemcisine teklif mesajı gönderir
 ==============================================================================}
...
begin
  ...
  Inc(p1);
  IPAdres := Isaretci(p1);
  IPAdres^ := GBaglantilar.AktifBaglanti.IP4Adres;
  Inc(IPAdres);
  p1 := Isaretci(IPAdres);
  ...
  // alt ağ maskesi - 2 + 4 byte
  p1^ := DHCP_SECIM_ALTAG_MASKESI;
  Inc(p1);
  p1^ := 4;
  Inc(p1);
  IPAdres := Isaretci(p1);
  IPAdres^ := GBaglantilar.AktifBaglanti.AltAgMaskesi;
  Inc(IPAdres);
  p1 := Isaretci(IPAdres);
  DHCPYapiUzunlugu := DHCPYapiUzunlugu + 2 + 4;
  ...
  B := GBaglantilar.BaglantiOlustur(itIP4, btAktif, ptUDP, IPAdresi, DHCP_SUNUCU_PORT,
    DHCP_ISTEMCI_PORT);
  if not(B = nil) then
  begin
...
{==============================================================================
  DHCP sunucusuna istek mesajı gönderir
 ==============================================================================}
procedure TDHCPv4.DHCPIstekMesajiGonder(ADHCPSunucuIPAdresi, AIstenenIPAdresi: TIP4Adres);
var
  B: TGenelBaglanti;
  DHCPYapi: PDHCP4Yapi;
  IPAdres: PIP4Adres;
...
begin
  ...
  // tam bilgisayar adı - (domain adı dahil)
  i := Length(GTamBilgisayarAdi);
...
{==============================================================================
  DHCP istemcisinin istek mesajına onay yanıtı gönderir
 ==============================================================================}
...
begin
  ...
  // alan adı - 2 byte + ad uzunluğu + 1 byte
  i := Length(GAlanAdi);
  p1^ := DHCP_SECIM_ALAN_ADI;
  Inc(p1);
  PByte(p1)^ := i + 1;
...
{==============================================================================
  DHCP sunucusuna bilgilendirme mesajı gönderir
 ==============================================================================}
procedure TDHCPv4.DHCPBilgilendirmeMesajiGonder(AIstemciIPAdres: TIP4Adres);
var
  B: TGenelBaglanti;
  DHCPYapi: PDHCP4Yapi;
  MACAdres: PMACAdres;
...
{==============================================================================
  DHCP istemcisinin bilgilendirme mesajına onay yanıtı gönderir
 ==============================================================================}
...
{==============================================================================
  DHCP istemcisine ret mesajı gönderir
 ==============================================================================}
